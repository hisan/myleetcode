...
int wiggleMaxLength(int* nums, int numsSize)
{
    ...
    int notsame = 0;
    for (int i = 0 ; i < numsSize -1;i++)//排除数组元素全部相等的情况
    {
        if (nums[i] != nums[i+1])
        {
            notsame = 1;
    ...
    {
        if (flag[i] == flag[i+1] && flag[i] != 0)//针对> > 或者 < <
        {
            sublen -= 1;
        }
        ...
        if (i > 0 && i < numsSize - 2)
        {
            if (flag[i] == 0 && (flag[i-1] == flag[i+1]))//针对> 0 > 或者 < 0 <
            {
                sublen -= 1;
            }
        }
...
int main()
{
	...
	//int nums[7] = {1,1,7,4,9,2,5};
	//int nums[5] ={3,3,3,2,5};	
	...
	int tmp = wiggleMaxLength(nums,sizeof(nums)/sizeof(int));
	printf("%d\r\n",tmp);
	return 0;
}
